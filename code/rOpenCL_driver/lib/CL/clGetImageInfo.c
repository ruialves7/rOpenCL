...
POname(clGetImageInfo)(cl_mem image, cl_image_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret)
    CL_API_SUFFIX__VERSION_1_0
{
#if DEBUG == 0
    puts("--- Start execute clGetImageInfo primitive\n ---");
#endif
    char id = 0x31;
    ...
    size_buffer_data_request = sizeof(cl_mem) + sizeof(cl_image_info) + sizeof(size_t);
#if PROTOCOL == 1
    ...
    _ccl_memcpy(buffer_data_request, &param_name, sizeof(cl_image_info), &offset_buffer);
    buffer_data_request += sizeof(cl_image_info);
    ...
    init_udp(&fd, &addr_local, &addr, "clGetImageInfo");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clGetImageInfo");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetImageInfo") == 1)
    {
    ...
    closeSocketUdp(&fd);
#else
    int fd_connect = init_tcp(&fd, &addr, "clGetImageInfo");
    // header = build_header_tcp(id, &size_buffer_data_request);
    send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clGetImageInfo");
    recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetImageInfo");
...
#if DEBUG == 0
    puts("--- End execute clGetImageInfo primitive\n ---");
#endif
    return (result);
}
