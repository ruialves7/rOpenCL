...
{
#if DEBUG == 0
        puts("--- Start execute clEnqueueWriteBufferRect primitive\n ---");    
#endif
    char id = 0x1B;
    struct sockaddr_in addr;
    void * buffer_data_request = NULL, *buffer_data_reply = NULL, *header = NULL, *ptr_ = NULL;
    int size_ptr = 0, x = 0, y = 0, fd = 0, size_buffer_data_request = 0, size_buffer_data_reply = 0, offset_buffer = 0;
    cl_int result_;
    cl_event event_;
    ...
    if (ptr_ != NULL) {
        ...
        size_buffer_data_request = sizeof (cl_bool) + (1 * sizeof (char)) + size_ptr + sizeof (cl_command_queue) + sizeof (cl_mem) + (13 * sizeof (size_t)) + sizeof (cl_uint);
#if PROTOCOL == 1
    ...
    buffer_data_request += sizeof (int);
        ...
        if (size_ptr > 0)
            size_buffer_data_request += size_ptr;
        ...
        {
            void *sss = (void*)ptr;
            _ccl_memcpy(buffer_data_request, sss, size_ptr, &offset_buffer);
            buffer_data_request += size_ptr;
        }
        ...
        init_udp(&fd, &addr_local,&addr,  "clEnqueueWriteBufferRect");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clEnqueueWriteBufferRect");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueWriteBufferRect") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clEnqueueWriteBufferRect");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clEnqueueWriteBufferRect");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueWriteBufferRect");
...
#if DEBUG == 0
        puts("--- End execute clEnqueueWriteBufferRect primitive\n ---");    
#endif
    return (result_);  
}
