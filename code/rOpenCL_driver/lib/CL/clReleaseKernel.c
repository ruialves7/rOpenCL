...
{
#if DEBUG == 0
        puts("--- Start execute clReleaseKernel primitive\n ---");    
#endif
    char id = 0x5D;
    ...
    buffer_data_request -= offset_buffer;
        ...
        init_udp(&fd, &addr_local, &addr,"clReleaseKernel");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clReleaseKernel");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clReleaseKernel") == 1) 
        {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clReleaseKernel");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clReleaseKernel");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clReleaseKernel");
        closeSocketTCP(&fd,&fd_connect);
#endif
        free(buffer_data_request);

#if DEBUG == 0
        puts("--- End execute clReleaseKernel primitive\n ---");    
#endif
