...
POname(clSetUserEventStatus)(cl_event event,cl_int execution_status ) CL_API_SUFFIX__VERSION_1_1
{
#if DEBUG == 0
        puts("--- Start execute clSetUserEventStatus primitive\n ---");    
#endif
    char id = 0x61;
    ...
    } else 
    {
        ...
        size_buffer_data_request = sizeof (cl_event) + sizeof (cl_int);
#if PROTOCOL == 1
    ...
    buffer_data_request += sizeof (int);
        ...
        _ccl_memcpy(buffer_data_request, &execution_status, sizeof (cl_int), &offset_buffer);
        buffer_data_request += sizeof (cl_int);
        ...
        init_udp(&fd, &addr_local, &addr, "clSetUserEventStatus");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clSetUserEventStatus");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clSetUserEventStatus") == 1) 
        {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clSetUserEventStatus");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clSetUserEventStatus");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clSetUserEventStatus");
        ...
        free(buffer_data_reply);
#if DEBUG == 0
        puts("--- end execute clSetUserEventStatus primitive\n ---");    
#endif
    return (result_);
}
