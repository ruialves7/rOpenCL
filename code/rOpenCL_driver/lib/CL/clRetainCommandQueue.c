...
{
#if DEBUG == 0
        puts("--- Start execute clRetainCommandQueue primitive\n ---");    
#endif
    char id = 0x2D;
    ...
    buffer_data_request -= offset_buffer;
        ...
        init_udp(&fd, &addr_local, &addr, "clRetainCommandQueue");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clRetainCommandQueue");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainCommandQueue") == 1) {
        ...
        int fd_connect = init_tcp(&fd, &addr, "clRetainCommandQueue");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clRetainCommandQueue");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clRetainCommandQueue");
...
#if DEBUG == 0
        puts("--- End execute clRetainCommandQueue primitive\n ---");    
#endif
