...
POname(clEnqueueMarkerWithWaitList) (cl_command_queue  command_queue,cl_uint num_events_in_wait_list,const cl_event * event_wait_list,cl_event * event) CL_API_SUFFIX__VERSION_1_2
{
    
#if DEBUG == 0
        puts("--- Start execute clEnqueueMarkerWithWaitList primitive\n ---");    
#endif
    char id = 0x6B;
    ...
    buffer_data_request += sizeof (int);
        ...
        init_udp(&fd, &addr_local,&addr,  "clEnqueueMarkerWithWaitList");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clEnqueueMarkerWithWaitList");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueMarkerWithWaitList") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clEnqueueMarkerWithWaitList");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clEnqueueMarkerWithWaitList");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueMarkerWithWaitList");
...
#if DEBUG == 0
        puts("--- End execute clEnqueueMarkerWithWaitList primitive\n ---");    
#endif
    return (result_);     
}
