...
{
#if DEBUG == 0
    puts("--- Start execute clReleaseProgram primitive\n ---");
#endif
    char id = 0x51;
    ...
    } ccl_request = {.program = obj->object_remote};
    ...
    init_udp(&fd, &addr_local, &addr, "clReleaseProgram");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clReleaseProgram");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clReleaseProgram") == 1)
    {
    ...
    closeSocketUdp(&fd);
#else
    int fd_connect = init_tcp(&fd, &addr, "clReleaseProgram");
    send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clReleaseProgram");
    recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clReleaseProgram");
...
#if DEBUG == 0
    puts("--- End execute clReleaseProgram primitive\n ---");
#endif
