...
{
#if DEBUG == 0
        puts("--- Start execute clRetainContext primitive\n ---");    
#endif
    char id = 0x27;
    ...
    buffer_data_request += sizeof (int);
        ...
        init_udp(&fd, &addr_local, &addr, "clRetainContext");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clRetainContext");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainContext") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clRetainContext");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clRetainContext");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clRetainContext");
...
#if DEBUG == 0
        puts("--- End execute clRetainContext primitive\n ---");    
#endif
