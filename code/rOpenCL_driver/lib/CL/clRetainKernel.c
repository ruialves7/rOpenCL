...
{
#if DEBUG == 0
        puts("--- Start execute clRetainKernel primitive\n ---");    
#endif
    char id = 0x5C;
    ...
    buffer_data_request -= offset_buffer;
        ...
        init_udp(&fd, &addr_local, &addr, "clRetainKernel");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clRetainKernel");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainKernel") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clRetainKernel");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clRetainKernel");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clRetainKernel");
...
#if DEBUG == 0
        puts("--- End execute clRetainKernel primitive\n ---");    
#endif
