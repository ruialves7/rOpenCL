...
POname(clGetDeviceInfo)(cl_device_id  device, cl_device_info param_name, size_t param_value_size, void *param_value,size_t *      param_value_size_ret) CL_API_SUFFIX__VERSION_1_0
{
#if DEBUG == 0
        puts("--- Start execute clGetDeviceInfo primitive\n ---");    
#endif
    char id = 0x05;
    ...
    if (ptr != NULL) {
        ...
        size_buffer_data_request = sizeof (cl_device_id) + sizeof (cl_device_info) + sizeof (size_t);
#if PROTOCOL == 1
    ...
    buffer_data_request += sizeof (size_t);
        ...
        _ccl_memcpy(buffer_data_request, &param_name, sizeof (cl_device_info), &offset_buffer);
        buffer_data_request += sizeof (cl_device_info);
       ...
       char *id_transaction = NULL;
        ...
        init_udp(&fd, &addr_local,&addr, "clGetDeviceInfo");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clGetDeviceInfo");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetDeviceInfo") == 1) 
        {
        ...
        int  fd_connect = init_tcp(&fd, &addr, "clGetDeviceInfo");
     
        //header = build_header_tcp(id, &size_buffer_data_request);
     
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clGetDeviceInfo");
      
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetDeviceInfo");
        ...
        if (result == CL_SUCCESS) 
        {
            ...
            if (param_value != NULL) 
            {
                ...
                {
                    cl_platform_id pp,pq; 
                    memcpy(&pp,_param_value , _param_value_size_ret);   
                    
                     ptr  = register_object(pp,getDispachPointer(0),obj->daemon);
                     obj = *(cl_opencl_object**)ptr;
                     pq= obj->object_local;
                     memcpy(param_value, &pq, sizeof(cl_platform_id));
...
#if DEBUG == 0
        puts("--- End execute clGetDeviceInfo primitive\n ---");    
#endif
    return (result);
}
