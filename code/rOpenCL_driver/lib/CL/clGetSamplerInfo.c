...
POname(clGetSamplerInfo)(cl_sampler sampler, cl_sampler_info param_name, size_t param_value_size, void *param_value, size_t *param_value_size_ret) CL_API_SUFFIX__VERSION_1_0
{
#if DEBUG == 0
    puts("--- Start execute clGetSamplerInfo primitive\n ---");
#endif
    char id = 0x4C;
    ...
    size_buffer_data_request = sizeof(cl_sampler) + sizeof(cl_sampler_info) + sizeof(size_t);
#if PROTOCOL == 1
    ...
    _ccl_memcpy(buffer_data_request, &param_name, sizeof(cl_sampler_info), &offset_buffer);
    buffer_data_request += sizeof(cl_sampler_info);
    ...
    init_udp(&fd, &addr_local, &addr, "clGetSamplerInfo");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clGetSamplerInfo");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetSamplerInfo") == 1)
    {
    ...
    closeSocketUdp(&fd);
#else
    int fd_connect = init_tcp(&fd, &addr, "clGetSamplerInfo");
    // header = build_header_tcp(id, &size_buffer_data_request);
    send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clGetSamplerInfo");
    recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetSamplerInfo");
...
#if DEBUG == 0
    puts("--- End execute clGetSamplerInfo primitive\n ---");
#endif
    return (result);
}
