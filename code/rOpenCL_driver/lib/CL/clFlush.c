...
{
#if DEBUG == 0
        puts("--- Start execute clFlush primitive\n ---");    
#endif
    char id = 0x43;
    ...
    buffer_data_request -= offset_buffer;
        ...
        init_udp(&fd, &addr_local, &addr, "clFlush");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clFlush");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clFlush") == 1) {
         ...
         closeSocketUdp(&fd);
#else
        int fd_connect  = init_tcp(&fd, &addr, "clFlush");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clFlush");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clFlush");
        closeSocketTCP(&fd,&fd_connect);
#endif
        free(buffer_data_request);

#if DEBUG == 0
        puts("--- End execute clFlush primitive\n ---");    
#endif
