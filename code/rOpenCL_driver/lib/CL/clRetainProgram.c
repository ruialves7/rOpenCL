...
{
#if DEBUG == 0
    puts("--- Start execute clRetainProgram primitive\n ---");
#endif
    char id = 0x50;
    ...
    init_udp(&fd, &addr_local, &addr, "clRetainProgram");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clRetainProgram");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainProgram") == 1)
    {
    ...
    closeSocketUdp(&fd);
#else
    int fd_connect = init_tcp(&fd, &addr, "clRetainProgram");
    send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clRetainProgram");
    recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clRetainProgram");
...
#if DEBUG == 0
    puts("--- End execute clRetainProgram primitive\n ---");
#endif
