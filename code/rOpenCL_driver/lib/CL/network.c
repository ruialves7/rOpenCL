...
int recv_data_tcp(int *fd, void *buffer, size_t size, char *primitive)
{
    size_t size_read = 0, return_sendto = 0, size_recv = 0, offset_total = 0;

    return_sendto = recv(*fd, &size_recv, sizeof(size_t), 0);

    size_t rsize = size_recv - sizeof(size_t);
