...
POname(clGetMemObjectInfo)(cl_mem memobj ,cl_mem_info param_name , size_t param_value_size ,void * param_value ,size_t * param_value_size_ret) CL_API_SUFFIX__VERSION_1_0
{
   
#if DEBUG == 0
        puts("--- Start execute clGetMemObjectInfo primitive\n ---");    
#endif
    ...
    if (ptr  != NULL) {
       ...
       size_buffer_data_request = sizeof (cl_mem) + sizeof (cl_mem_info) + sizeof (size_t);
#if PROTOCOL == 1
    ...
    buffer_data_request += sizeof (int);
        ...
        _ccl_memcpy(buffer_data_request, &param_name, sizeof (cl_mem_info), &offset_buffer);
        buffer_data_request += sizeof (cl_mem_info);
        ...
        size_buffer_data_reply = sizeof (cl_int) + sizeof (size_t) + param_value_size;
        ...
        init_udp(&fd, &addr_local, &addr, "clGetMemObjectInfo");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clGetMemObjectInfo");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetMemObjectInfo") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clGetMemObjectInfo");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clGetMemObjectInfo");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clGetMemObjectInfo");
...
#if DEBUG == 0
        puts("--- End execute clGetMemObjectInfo primitive\n ---");    
#endif
    return (result);
}
