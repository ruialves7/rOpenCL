#include "pocl_cl.h"
CL_API_ENTRY cl_int CL_API_CALL POname (clRetainSampler) (cl_sampler sampler)CL_API_SUFFIX__VERSION_1_0
{
#if DEBUG == 0
        puts("--- Start execute clRetainSampler primitive\n ---");    
#endif
    char id = 0x4A;
    ...
    buffer_data_request += sizeof (int);
        ...
        init_udp(&fd, &addr_local, &addr, "clRetainSampler");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clRetainSampler");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainSampler") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clRetainSampler");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clRetainSampler");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clRetainSampler");
        ...
        free(buffer_data_reply);
#if DEBUG == 0
        puts("--- End execute clRetainSampler primitive\n ---");    
#endif
    return (result);
}
