...
CL_API_ENTRY
cl_int CL_API_CALL
POname(clEnqueueBarrierWithWaitList)(cl_command_queue command_queue, cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) CL_API_SUFFIX__VERSION_1_2
{
#if DEBUG == 0
    puts("--- Start execute clEnqueueBarrierWithWaitList primitive\n ---");
#endif
    char id = 0x6C;
    ...
    size_buffer_data_request = (1 * sizeof(char)) + sizeof(cl_command_queue) + sizeof(cl_uint);
    ...
    init_udp(&fd, &addr_local, &addr, "clEnqueueBarrierWithWaitList");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clEnqueueBarrierWithWaitList");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueBarrierWithWaitList") == 1)
    {
    ...
    int fd_connect = init_tcp(&fd, &addr, "clEnqueueBarrierWithWaitList");
    // header = build_header_tcp(id, &size_buffer_data_request);
    send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clEnqueueBarrierWithWaitList");
    recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueBarrierWithWaitList");
...
#if DEBUG == 0
    puts("--- End execute clEnqueueBarrierWithWaitList primitive\n ---");
#endif
    return (result_);
}
