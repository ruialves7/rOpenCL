...
{
#if DEBUG == 0
        puts("--- Start execute clReleaseCommandQueue primitive\n ---");    
#endif
    char id = 0x2E;
    ...
    buffer_data_request += sizeof (int);
        ...
        init_udp(&fd, &addr_local, &addr, "clReleaseCommandQueue");
        ...
        send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clReleaseCommandQueue");
        free(id_transaction);

        if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clReleaseCommandQueue") == 1) {
        ...
        closeSocketUdp(&fd);
#else
        int fd_connect = init_tcp(&fd, &addr, "clReleaseCommandQueue");
        send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clReleaseCommandQueue");
        recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clReleaseCommandQueue");
        closeSocketTCP(&fd,&fd_connect);
#endif
        free(buffer_data_request);

#if DEBUG == 0
        puts("--- End execute clReleaseCommandQueue primitive\n ---");    
#endif
