...
POname(clEnqueueTask)(cl_command_queue command_queue,cl_kernel kernel,cl_uint num_events_in_wait_list,const cl_event *  event_wait_list, cl_event * event) CL_API_SUFFIX__VERSION_1_0
{
#if DEBUG == 0
        puts("--- Start execute clEnqueueTask primitive\n ---");    
#endif
    char id = 0x6F;
    ...
    if (ptr != NULL) 
    {
        ...
        size_buffer_data_request = sizeof (cl_kernel)+(1 * sizeof (char)) + sizeof (cl_command_queue) + sizeof (cl_uint);
    ...
    if (num_events_in_wait_list > 0) 
        ...
        init_udp(&fd, &addr_local, &addr,  "clEnqueueTask");
        ...
        int fd_connect = init_tcp(&fd, &addr, "clEnqueueTask");
        //header = build_header_tcp(id, &size_buffer_data_request);
        send_data_tcp(&fd, buffer_data_request, header, size_buffer_data_request, "clEnqueueTask");
        recv_data_tcp(&fd, buffer_data_reply, size_buffer_data_reply, "clEnqueueTask");
...
#if DEBUG == 0
        puts("--- End execute clEnqueueTask primitive\n ---");    
#endif
    return (result_);
}
POsym(clEnqueueTask)
