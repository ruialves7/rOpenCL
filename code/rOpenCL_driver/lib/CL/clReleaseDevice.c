...
{
#if DEBUG == 0
    puts("--- Start execute clReleaseDevice primitive\n ---");
#endif
    char id = 0x25;
    ...
    init_udp(&fd, &addr_local, &addr, "clReleaseDevice");
    ...
    send_data_udp(&fd, buffer_data_request, header, size_buffer_data_request, &addr, "clReleaseDevice");
    free(id_transaction);

    if (recv_data_udp(&fd, buffer_data_reply, size_buffer_data_reply, "clReleaseDevice") == 1)
    {
    ...
    closeSocketUdp(&fd);
#else
    int fd_connect = init_tcp(&fd, &addr, "clReleaseDevice");
    send_data_tcp(&fd, buffer_data_request, NULL, size_buffer_data_request, "clReleaseDevice");
    recv_data_tcp(&fd, &ccl_reply, sizeof(ccl_reply), "clReleaseDevice");
...
#if DEBUG == 0
    puts("--- End execute clReleaseDevice primitive\n ---");
#endif
